...
public:
  SATSolverDPLL() {}
  void initialize(vector< vector<int>>&,int); // intiializes the values
  void solve();      // calls the solver
  vector<int> getu(void);
};
/*
 * get the member data _u
 */

vector<int> SATSolverDPLL::getu(void){
	return _u;
}

/*
 * function that accepts the inputs from the CNF clause and initializes the attributes
 * in the solver
 */
void SATSolverDPLL::initialize(vector<vector<int>>& in,int literal_count) {

  clause_count = in.size();
...
//        cout << pow(-1, f.literals[i]) * (i + 1);
	_u.push_back(pow(-1, f.literals[i]) * (i+1));
